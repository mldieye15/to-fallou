      ...
      <v-row class="mb-0 mx-auto pa-1"  align="center">
        ...
        <v-col cols="auto">
          <!-- <v-btn variant="outlined" color="black" >
            <router-link :to="{ name: 'etablissement-add' }" class="">
      ...
      >
        <template #item-actions="item">
          <div class="actions-wrapper">
            <router-link :to="{ name: 'etablissement-details', params: { id: item.id } }"> <v-icon small flat color="green dark">mdi-eye</v-icon> </router-link>
            <router-link :to="{ name: 'etablissement-edit', params: { id: item.id } }" class="ml-4"> <v-icon small flat color="blue dark">mdi-pencil</v-icon> </router-link>
            <v-dialog  transition="dialog-top-transition" width="50%" height="auto">
              ...
              <template v-slot:default="{ isActive }">
                <v-card>
                  <v-toolbar color="primary" :title="$t('apps.forms.etablissement.etablissement')"></v-toolbar>
                  <v-card-text>
...
const redirectToAdd= () => {
  router.push({ name: 'etablissement-add'});
};
...
const { dataListeEtab, headerTable, loading } = storeToRefs(etablissementStore);
const { all, destroy } = etablissementStore;
