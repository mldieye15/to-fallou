<template>
  <div>
    <p class="text-h6">{{ $t('apps.forms.typeCentre.typeCentre') }}</p>
      ...
      <v-row class="mb-0 mx-auto pa-1"  align="center">
        ...
        <v-col cols="auto">
          <!-- <v-btn variant="outlined" color="black" >
            <router-link :to="{ name: 'typeCentre-add' }" class="">
      ...
      >
        <template #item-actions="item">
          <div class="actions-wrapper">
            <router-link :to="{ name: 'typeCentre-details', params: { id: item.id } }"> <v-icon small flat color="green dark">mdi-eye</v-icon> </router-link>
            <router-link :to="{ name: 'typeCentre-edit', params: { id: item.id } }" class="ml-4"> <v-icon small flat color="blue dark">mdi-pencil</v-icon> </router-link>
            <v-dialog  transition="dialog-top-transition" width="50%" height="auto">
              ...
              <template v-slot:default="{ isActive }">
                <v-card>
                  <v-toolbar color="primary" :title="$t('apps.forms.typeCentre.typeCentre')"></v-toolbar>
                  <v-card-text>
...
const redirectToAdd= () => {
  router.push({ name: 'typeCentre-add'});
};
...
const { dataListeTypeCentre, headerTable, loading } = storeToRefs(typeCentreStore);
const { all, destroy } = typeCentreStore;
