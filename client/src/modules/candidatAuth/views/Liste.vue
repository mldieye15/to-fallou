<template>
  <div>
    <p class="text-h6">Liste des candidats authorisés</p>
      ...
      <v-row class="mb-0 mx-auto pa-1"  align="center">
        ...
        <v-col cols="auto">
          <!-- <v-btn variant="outlined" color="black" >
            <router-link :to="{ name: 'candidatAuthoriser-add' }" class="">
          ...
          <v-btn @click.prevent="redirectToExlu()" class="ml-2" variant="flat" color="red-accent-4"> Candidats non authorisés</v-btn>
        </v-col>
      </v-row>
      <EasyDataTable
        :headers="headerTable"
...
const { dataListe, headerTable, loading } = storeToRefs(candidatAuthoriserStore);
const { all, destroy } = candidatAuthoriserStore;
