      ...
      >
        <template #item-actions="item">
          <div class="actions-wrapper">
            <router-link :to="{ name: 'typeSession-details', params: { id: item.id } }"> <v-icon small flat color="green dark">mdi-eye</v-icon> </router-link>
            <router-link :to="{ name: 'typeSession-edit', params: { id: item.id } }" class="ml-4"> <v-icon small flat color="blue dark">mdi-pencil</v-icon> </router-link>
            <v-dialog  v-model="dialog" transition="dialog-top-transition" width="50%" height="auto">
              ...
              <template v-slot:default="{ isActive }">
                <v-card>
                  <v-toolbar color="primary" :title="$t('apps.forms.typeSession.typeSession')"></v-toolbar>
                  <v-card-text>
...
const { dataListeTypeSession, headerTable, loading } = storeToRefs(typeSessionStore);
const { all, destroy } = typeSessionStore;
