        ...
        <template #table-row="props">
          <div v-if="props.column.field === 'totalDemandes'">
            <div v-if="props.row.quota===0">
              ...
              <router-link :to="{ name: 'demandeSecondaryVille-demandes', params: { id: props.row.id } }"> <v-icon class="ml-3" small flat color="green dark">mdi-eye</v-icon> </router-link>
            </div>
            <div v-else>
...
const { all, destroy,allSecondaryVille } = villeStore;
...
const redirectToDemandes = (id) => {
  router.push({ name: 'demandeSecondaryVille-demandes', params: { id } });
};
</script>
<style>
.v-text-field {
