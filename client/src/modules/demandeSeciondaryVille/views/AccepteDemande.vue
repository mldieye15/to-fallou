<template>
  <div v-if="inputForm.etatDemande==='en attente' ||inputForm.etatDemande==='déclinée' && inputForm.etatUser===false">
    ...
    <v-autocomplete
        ...
        variant="outlined"
        :items="dataListeByVilleSeconadry"
        persistent-hint
  ...
  import { useDemandeByVilleStore } from "@/modules/demandeByVille/store";
...
const { dataListeCentre,dataListeByVilleSeconadry} = storeToRefs(centreStore);
  ...
  const redirectToVilleAfterAccept = (villeId) => {
    router.push({ name: 'demandeSecondaryVille-demandes', params: { id: villeId } });
};
...
onMounted(() => {
  one(route.params.id ).then(() => {
    ...
    centreStore.centresByVilleSecondary(villeId);
    console.log("centre secondaire",dataListeByVilleSeconadry);
