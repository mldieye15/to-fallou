...
public interface JuryDao extends JpaRepository<Jury, Long> {
    ...
    @Query("SELECT j FROM Jury j WHERE j.numero= :numero AND j.session.sessionOuvert = true ")
    Optional<Jury> findByNumero(@Param("numero") String numero);
    @Query("SELECT j FROM Jury j WHERE j.numero= :numero AND j.id != :juryId AND j.session.sessionOuvert = true ")
